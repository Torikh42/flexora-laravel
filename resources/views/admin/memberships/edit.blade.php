...
@section('title', 'Edit Membership - Admin Flexora')
@section('page-title', 'Edit Membership')

@section('content')
<div class="max-w-2xl">
    <form action="{{ route('admin.memberships.update', $membership) }}" method="POST" class="bg-white rounded-lg shadow-md p-6">
        @csrf
        @method('PUT')
        ...
        <div class="mb-6">
            ...
            <input type="number" name="price" id="price" value="{{ old('price', $membership->price) }}" required min="0"
        ...
        <div class="mb-6">
            ...
            <input type="number" name="duration_days" id="duration_days" value="{{ old('duration_days', $membership->duration_days) }}" required min="1"
        ...
        <div class="mb-6">
            ...
            <textarea name="description" id="description" rows="3"
                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent @error('description') border-red-500 @enderror">{{ old('description', $membership->description) }}</textarea>
            @error('description')
                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
            @enderror
        </div>
