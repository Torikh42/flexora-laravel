...
<div class="px-4 md:px-8">
  <div class="bg-linear-to-b from-gray-300 to-gray-200 rounded-[30px] p-6 md:p-8 shadow-lg min-h-[500px] my-5">
    ...
    <div id="classGrid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 mb-10"></div>
  </div>
</div>
...
<script>
  ...
  function displayEnrollments(enrollments) {
    ...
    enrollments.forEach(enrollment => {
      ...
      let imageContent;
      if (enrollment.class_image) {
        imageContent = `<img src="${enrollment.class_image}" alt="${enrollment.class_name}" class="w-full h-[150px] object-cover" style="display: block;" />`;
      } else {
        imageContent = `<div class="w-full h-[150px] bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-5xl text-white">${emoji}</div>`;
      }
      ...
      if (imgElement) {
        imgElement.addEventListener('error', function() {
          const emojiSymbol = getClassEmoji(enrollment.class_name);
          this.replaceWith(
            Object.assign(document.createElement('div'), {
              className: 'w-full h-[150px] bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-5xl text-white',
              innerHTML: emojiSymbol
            })
          );
        });
      }
